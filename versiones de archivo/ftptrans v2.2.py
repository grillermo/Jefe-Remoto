...
machines = ['192.168.1.109','192.168.1.178']
file = ''
...
class Controller(GenericThread):
    ...
    def validateIP(self):
        ...
        try:
            ...
            who.selectAll()
        self.reporter.append(str(who.__name__()))

    def threadoutPut(self):
        text = self.qprocess.readAllStandardOutput()
        self.reporter.append(str(text))
    ##    self.qprocess.readAllStandardOutput()
    ##    self.qprocess.readAllStandardError()

    def setRange(self,ip):
        self.initialIP = ip
    ...
    def pinger(self,machines,timeout=1):
        for ip in machines:
            self.qprocess = QProcess()
            self.qprocess.connect(self.qprocess, SIGNAL( 'readyReadStandardOutput()' ),
                          self.threadoutPut )
            self.qprocess.connect(self.qprocess, SIGNAL( 'readyReadStandardError()' ),
                          self.threadoutPut  )
            self.qprocess.start( 'fping ',[ip])
...
##        error,returnStr = currentPing.communicate()
##        self.reporter.append(returnStr+'lol')
##            self.emit( SIGNAL('update'), returnStr)


##        for i in machines:
##            i = str(i)
##            try:
##                result = ping.do_one(i,timeout=9)
##                if type(result) is float:
##                    returnStr = i+ ' rocked my world baby'
##                else:
##                    raise
##            except:
##                returnStr = i+' failed to connect'
##            self.emit( SIGNAL('update'), returnStr)

    def multithreading(self,function,*args):
    ...
    def runJob(self):
        if self.mode == 'UE':
            ...
            self.multithreading(self.pinger,(machines))
...
class View(QDialog,Controller):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(self.interruptButton,12,1)
        layout.addWidget(self.reporter,13,0,1,0)
        layout.addWidget(self.closeButton,14,0)
